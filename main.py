...
def main():
    ...
    try:
        builtin_text = read_from_file_builtin('data/Titanic-Dataset.csv')
        output_to_console("\nContent of Titanic-Dataset.csv read using built-in functions:")
        output_to_console(builtin_text[:300] + "\n")
        write_to_file_builtin(builtin_text, 'data/Builtin_Titanic-Dataset.csv')
        output_to_console("Content written to data/Builtin_Titanic-Dataset.csv using built-in functions.")
    except Exception as e:
        output_to_console("An error occurred with built-in file functions: " + str(e))

    try:
        pandas_data = read_from_file_pandas('data/Titanic-Dataset.csv')
        output_to_console("Content of Titanic-Dataset.csv read using pandas:")
        output_to_console(pandas_data.head().to_string())
        write_to_file_pandas(pandas_data.head(), 'data/Pandas_Titanic-Dataset.csv')
        output_to_console("Content written to data/Pandas_Titanic-Dataset.csv using pandas. ")
    except Exception as e:
